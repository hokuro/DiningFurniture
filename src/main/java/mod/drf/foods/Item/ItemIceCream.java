...
public class ItemIceCream extends ItemResetFood {
	...
	public ActionResult<ItemStack> onItemRightClick(ItemStack itemstack, World world, EntityPlayer player, EnumHand hand){
		...
		if (movingbjectposition == null){
			// 置くブロックがないのでそのまま食べる
	        if (player.canEat(ConfigValue.General.IceCreamCanEatAllways) || ModCommon.isDeveloped)
	        {
		...
		if (movingbjectposition.typeOfHit == RayTraceResult.Type.BLOCK){
			...
			if (world.isAirBlock(blockpos.add(0,1,0))){
				if (!world.isRemote){
					try{
						EntityIceCream ice = new EntityIceCream(world, blockpos.add(0, 1, 0), itemstack.getItemDamage());
